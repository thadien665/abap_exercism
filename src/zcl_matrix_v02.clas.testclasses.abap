...
CLASS ltcl_matrix DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_matrix_v02.
    METHODS setup.
...
CLASS ltcl_matrix IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_matrix_v02( ).
  ENDMETHOD.

  " extract row from one number matrix
  METHOD test_extract_row_one.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 1 ) ).
  ...
  METHOD test_extract_row_two.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 3 ) ( 4 ) ).
  ...
  METHOD test_extract_row_three.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 10 ) ( 20 ) ).
    DATA(actual_value) = cut->matrix_row( string = '1 2\n10 20'
                                          index  = 2 ).
    cl_abap_unit_assert=>assert_equals(
      act = actual_value
  ...
  METHOD test_extract_row_four.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 8 ) ( 7 ) ( 6 ) ).
  ...
  METHOD test_extract_column_two.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 3 ) ( 6 ) ( 9 ) ).
  ...
  METHOD test_extract_column_three.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 4 ) ( 8 ) ( 6 ) ).
  ...
  METHOD test_extract_column_four.
    DATA(exptected_value) = VALUE zcl_matrix_v02=>integertab( ( 1903 ) ( 3 ) ( 4 ) ).
